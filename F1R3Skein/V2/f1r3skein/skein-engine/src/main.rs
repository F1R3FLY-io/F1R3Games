...
const DEFAULT_ADDR: &str = "0.0.0.0:7643";
...
fn main() {
    ...
    let listener = match TcpListener::bind(&addr) {
        Ok(l) => l,
        Err(e) => {
            eprintln!("[engine] cannot bind {addr}: {e}");
            std::process::exit(1);
        }
    };
    eprintln!("[engine] protocol v{PROTOCOL_VERSION}, listening on {addr}");
    eprintln!("[engine] advertise this as _f1r3skein._tcp for Bonjour discovery");
