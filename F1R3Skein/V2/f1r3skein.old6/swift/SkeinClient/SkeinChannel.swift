...
public final class SkeinChannel {
    ...
    ///
    /// Written against a byte array rather than `Data` on purpose. `Data`
    /// indices do not rebase to zero after `removeSubrange`, so the obvious
    /// version faults on the *second* message in a buffer — which is the first
    /// moment the transport actually works, making it a bug that only appears
    /// once everything else is fixed.
    private func drainLines() {
        var bytes = [UInt8](inbox)
        var start = 0
        var consumed = 0
        while let nl = bytes[start...].firstIndex(of: 0x0A) {
            let line = Data(bytes[start..<nl])
            consumed = nl + 1
            start = consumed
            guard !line.isEmpty,
        ...
        if inbox.count > 4 * 1024 * 1024 {
            inbox.removeAll()
            state = .failed("oversized frame from engine")
        }
    }
