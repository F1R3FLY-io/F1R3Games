...
public enum ClientMessage: Encodable {
    ...
    case configure(
        pitchConstant: String?, pitchBase: Int?,
        pitchMap: String?, durationMap: String?,
        root: UInt8?, instrument: UInt8?)
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .configure(let constant, let base, let pm, let dm, let root, let inst):
            try c.encode("configure", forKey: .type)
            // The pitch stream: a base change is a stream change, and the
            // engine refuses one while zipped.
            if let constant, let base {
                try c.encode(SpigotConfigWire(constant: constant, base: base),
                             forKey: .left)
            } else {
...
/// Mirrors the Rust `SpigotConfig`.
public struct SpigotConfigWire: Encodable {
