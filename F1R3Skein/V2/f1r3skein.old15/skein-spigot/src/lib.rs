...
impl SpigotConfig {
    /// Bases run 2..=64.
    ///
    /// The specification says 2..=36, a limit that came from wantingeach digit to
    /// render as one alphanumeric character. The musical requirement is
    /// stronger: a pitch base is three octaves of a scale plus a rest, so
    /// `base = 3 * degrees + 1` — 16 for pentatonic, 22 for diatonic, and 37
    /// for chromatic, which the old cap excluded. Digit extraction is
    /// base-agnostic; only labelling was ever constrained.
    pub fn new(constant: Constant, base: u32) -> Result<Self, String> {
        if !(2..=64).contains(&base) {
            return Err(format!("base {base} out of range 2..=64"));
        }
...
mod tests {
    ...
    fn base_is_validated() {
        ...
        assert!(SpigotConfig::new(Constant::Pi, 65).is_err());
        assert!(SpigotConfig::new(Constant::Pi, 64).is_ok());
    }

    #[test]
    fn the_musical_bases_are_available() {
        // base = 3 * scale degrees + 1: three octaves plus a rest.
        for (base, degrees) in [(16u32, 5u32), (22, 7), (37, 12)] {
            assert_eq!(base, 3 * degrees + 1);
            let cfg = SpigotConfig::new(Constant::Pi, base).unwrap();
            let mut st = DigitStream::new(cfg);
            let d = st.range(0, 200);
            assert!(d.iter().all(|x| (*x as u32) < base));
            // The rest value must actually occur, or a third of the design is
            // unreachable.
            assert!(d.iter().any(|x| *x as u32 == base - 1), "base {base}");
        }
    }
}
