...
public final class SkeinModel {
    ...
    /// Base and scale are one choice: base = 3 * degrees + 1.
    public var scale: Scale = .diatonic

    /// A base change is a stream change, so the engine refuses it while
    /// zipped. The panel disables the control rather than letting M discover
    /// that by being refused.
    public var canChangeScale: Bool { !zip.zipped }

    public func setScale(_ s: Scale) {
        scale = s
        channel.send(ClientMessage.configure(
            pitchConstant: "pi", pitchBase: s.base,
            pitchMap: s.pitchMap, durationMap: nil,
            root: nil, instrument: nil))
    }
    ...
    public func setMaps(pitch: String, duration: String, root: UInt8) {
        ...
        channel.send(ClientMessage.configure(
            pitchConstant: nil, pitchBase: nil,
            pitchMap: pitch, durationMap: duration,
            root: root, instrument: nil))
    }
