...
fn handle(sess: &mut Session, msg: ClientMsg) -> Vec<EngineMsg> {
    match msg {
        ...
        ClientMsg::Configure {
            ...
            // Streams first: a base change is refused while zipped, and the
            // caller should hear about that rather than have the maps change
            // underneath an unchanged stream.
            if left.is_some() || right.is_some() {
                if let Outcome::Rejected(reason) =
                    sess.instrument.set_streams(left, right)
                {
                    return vec![EngineMsg::Rejected { v: PROTOCOL_VERSION, reason }];
                }
            }
...
fn selftest() {
    ...
    assert_eq!(a, b);
    // Two kinds of silence, and only one of them is captured.
    //
    // PERFORMED silence — a halt — consumes no notches, so it lives in the
    // performance trace and never appears in a tune. GENERATED silence — the
    // top digit of the pitch base — *is* a notch: addressed, bred, captured.
    // So the test is that the capture has exactly as many cells as notches,
    // not that none of them is silent.
    assert_eq!(
        a.len(),
        cap.notches,
        "a capture spanning a halt gains no cells from the silence"
    );
    let rests = a.iter().filter(|n| n.pitch.is_none()).count();
    println!(
        "  re-realised capture 1: {} notes ({rests} generated rests), reproducible",
        a.len()
    );
