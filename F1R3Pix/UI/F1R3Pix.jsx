...
// ══════════════════════════════════════════════════════════════════
// F1R3Pix — Collective Visual Intelligence
...
const PALETTE = [
  ...
  "#FF2D55","#FF375F","#FF6B2B","#FFD60A","#F3D630",
  "#30D158","#8BB999","#00C7BE","#40C8E0","#3FA9F5",
  "#5E5CE6","#5856D6","#BF5AF2","#AF52DE","#2e31ae",
];
    ...
    <div style={{minHeight:"100vh",background:BRAND.surface,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",fontFamily:FONT_HEADING}}>
      ...
      <div style={{animation:"fadeIn .6s ease-out",textAlign:"center"}}>
        ...
        <h1 style={{margin:"16px 0 0",fontSize:28,fontWeight:300,letterSpacing:6,color:"#888",fontFamily:FONT_HEADING}}>
          F1R3<span style={{color:BRAND.yellow,fontWeight:700}}>Pix</span>
        </h1>
        <div style={{fontSize:10,color:BRAND.textDim,marginTop:4,letterSpacing:2}}>COLLECTIVE VISUAL INTELLIGENCE</div>
        <div style={{marginTop:32,display:"flex",flexDirection:"column",gap:12,alignItems:"center"}}>
          ...
          <button onClick={go} style={{background:BRAND.yellow,color:"#111",border:"none",borderRadius:6,
            padding:"10px 32px",fontSize:13,fontFamily:FONT_HEADING,fontWeight:600,cursor:"pointer",letterSpacing:3,boxShadow:`0 0 20px ${BRAND.yellow}44`}}>JOIN</button>
        </div>
        {players.length>0&&(
          <div style={{marginTop:24,color:BRAND.textDim,fontSize:10}}>{players.length} playing</div>
        )}
      </div>
    </div>
  );
...
export default function F1R3Pix(){
  ...
  const[grid,setGrid]=useState(()=>{const g={};for(let y=0;y<GRID_H;y++)for(let x=0;x<GRID_W;x++)g[`${x},${y}`]={color:"#1C1C1E",by:null,ts:0};return g});
  const[selColor,setSelColor]=useState(PALETTE[9]);
  ...
  const loadGrid=useCallback(async()=>{
    const keys=await sh.list(PFX_PX);const g={...grid};
  ...
  const cellSize=Math.min(28,Math.floor((typeof window!=="undefined"?Math.min(window.innerWidth*.55,500):400)/GRID_W));
  ...
  return(
    <div style={{minHeight:"100vh",background:BRAND.surface,color:"#ddd",fontFamily:FONT_BODY,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <BrandBar title="Pix" accent={BRAND.yellow}
        subtitle={`${players.length} players | ${pixelCount} pixels | shard ${shardOk?"live":"offline"}`}
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:16,overflow:"auto"}}>
          <div style={{display:"grid",gridTemplateColumns:`repeat(${GRID_W},${cellSize}px)`,gap:1,background:"#111",padding:1,borderRadius:4}}>
            {Array.from({length:GRID_H},(_, y)=>
              Array.from({length:GRID_W},(_,x)=>{
                const key=`${x},${y}`;const px=grid[key];
                return(
                  <div key={key} onClick={()=>placePixel(x,y)}
                    onMouseEnter={()=>setHoverCell(key)} onMouseLeave={()=>setHoverCell(null)}
                    style={{width:cellSize,height:cellSize,background:px?.color||"#1C1C1E",
                      cursor:"pointer",transition:"background .1s",
                      outline:hoverCell===key?`2px solid ${selColor}`:"none",outlineOffset:-2,
                      borderRadius:1}}/>
                );
              })
            )}
        ...
        {/* Palette + info */}
        <div style={{width:180,borderLeft:`1px solid ${BRAND.border}`,padding:"16px 12px",display:"flex",flexDirection:"column",gap:16,overflowY:"auto"}}>
          <div style={{fontSize:10,color:BRAND.textMuted,letterSpacing:2,fontFamily:FONT_HEADING}}>PALETTE</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:4}}>
            {PALETTE.map(c=>(
              <div key={c} onClick={()=>setSelColor(c)}
                style={{width:28,height:28,borderRadius:4,background:c,cursor:"pointer",
                  border:selColor===c?"2px solid #fff":c==="#000000"?"1px solid #333":"1px solid transparent",
                  transition:"transform .1s",transform:selColor===c?"scale(1.15)":"scale(1)"}}/>
            ))}
          </div>

          <div style={{fontSize:10,color:BRAND.textMuted,letterSpacing:2,fontFamily:FONT_HEADING}}>ACTIVE</div>
          <div style={{fontSize:10,color:"#666",fontFamily:FONT_BODY}}>
          ...
          <div style={{fontSize:10,color:BRAND.textMuted,letterSpacing:2,fontFamily:FONT_HEADING,marginTop:8}}>PLAYERS</div>
          <div style={{display:"flex",flexDirection:"column",gap:4}}>
            {players.map(p=>(
              <div key={p.id} style={{display:"flex",alignItems:"center",gap:6,opacity:p.id===myId?1:.6}}>
                <Sigil seed={hSeed(p.id)} size={18}/>
                <span style={{fontSize:10,color:p.id===myId?BRAND.yellow:"#666",fontFamily:FONT_BODY}}>{p.name}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <footer style={{padding:"6px 20px",borderTop:`1px solid ${BRAND.border}`,display:"flex",
        ...
        <span>shard: {pixelCount} pixels</span>
